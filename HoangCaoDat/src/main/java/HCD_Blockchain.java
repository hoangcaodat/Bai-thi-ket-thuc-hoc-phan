...
public class HCD_Blockchain {
    public static ArrayList<VNPT_Dat> blockchain = new ArrayList<VNPT_Dat>();
    public static int difficulty = 5;
    public static float minimumTransaction = 0.1f;
    public static Store HCD_Kho; //Kho
    public static Store HCD_Cuahang; //Cửa hàng
    ...
    public static void main(String[] args) {
        ...
        genesisTransaction = new Transaction(coinbase.publicKey, HCD_Kho.publicKey, initBalanceA, null);
        ...
        VNPT_Dat genesis = new VNPT_Dat("0");
        genesis.addTransaction(genesisTransaction);
        addBlock(genesis);
        System.out.println("Số lượng VNPT-Net Router trong kho là : " + HCD_Kho.getBalance());
        System.out.print("Số lượng VNPT-Net Router của Cửa hàng đang còn : ");
        float initBalanceB = sc.nextFloat();
        genesisTransaction = new Transaction(coinbase.publicKey, HCD_Cuahang.publicKey, initBalanceB, null);
        genesisTransaction.generateSignature(coinbase.privateKey);
        genesisTransaction.transactionId = "0";
        genesisTransaction.outputs.add(new TransactionOutput(genesisTransaction.reciepient, genesisTransaction.value, genesisTransaction.transactionId)); //Thêm Transactions Output
        UTXOs.put(genesisTransaction.outputs.get(0).id, genesisTransaction.outputs.get(0));
        genesis.addTransaction(genesisTransaction);
        addBlock(genesis);
        System.out.println("Số lượng VNPT-Net Router của Cửa hàng đang còn là : " + HCD_Cuahang.getBalance());
        VNPT_Dat block1 = new VNPT_Dat(genesis.hash);
        boolean fail = true;
        while (fail){
            ...
            sendFund = HCD_Kho.sendFunds(HCD_Cuahang.publicKey, numberTransfer);
            if (sendFund==null){
                continue;
            }else{
                fail= false;
        ...
        System.out.println("Số lượng VNPT-Net Router trong kho hiện tại là : " + HCD_Kho.getBalance());
        System.out.println("Số lượng VNPT-Net Router của Cửa hàng hiện tại là : " + HCD_Cuahang.getBalance());
    }
    public static Boolean isChainValid() {
        VNPT_Dat currentBlock;
        VNPT_Dat previousBlock;
    ...
    public static void addBlock(VNPT_Dat newBlock) {
